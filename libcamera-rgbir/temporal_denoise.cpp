...
void TemporalDenoise::apply(uint16_t *frame, size_t count)
{
	 ...
	 */
	const unsigned int blocks = (count + kBlock - 1) / kBlock;
	blockMotion_.resize(blocks);
	/*
	 * Subsample the motion pass. A block is kBlock samples and we only need
	 * its mean |d| to a few counts, so every fourth sample is ample - the
	 * estimate is over 64 samples either way. Reading all of them made this
	 * pass cost as much as the filter itself, for no useful precision.
	 */
	...
	/*
	 * nth_element, not sort: only the 10th percentile is wanted and this is
	 * linear rather than n log n. The copy is still needed because the
	 * partition reorders, and blockMotion_ is indexed by block below.
	 */
	...
	for (unsigned int b = 0; b < blocks; b++) {
		...
		if (m < 0)
			m = 0;
		int32_t w = m >= thr ? 256 : aFix + (256 - aFix) * m / thr;
		if (w > 200)
			moving++;
		...
		for (size_t i = s0; i < s1; i++) {
			...
			const int32_t prev = history_[i];
			const int32_t out = prev + ((cur - prev) * w >> 8);
			const uint16_t v = static_cast<uint16_t>(out < 0 ? 0 : out);
			frame[i] = v;
			history_[i] = v;
		}
	}
