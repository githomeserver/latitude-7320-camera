...
static int skl_int3472_tps68470_probe(struct i2c_client *client)
{
	...
	case DESIGNED_FOR_WINDOWS:
		 ...
		 *
		 * On one manual insmod-after-boot the regulator cell failed with
		 * "-ENOENT: getting tps68470-clk" while the clk cell was bound,
		 * and hand-binding the regulator afterwards worked. Splitting
		 * this into two calls with wait_for_device_probe() in between
		 * would be the obvious fix, but wait_for_device_probe() does
		 * flush_work(&deferred_probe_work), which deadlocks if this
		 * probe is itself running on that workqueue - a real
		 * possibility at boot. Not worth risking a boot hang for a
		 * symptom only ever seen on a hand-loaded module, so measure it
		 * at boot first.
		 */
