...
#   sudo ./install-ccm.sh intel              all seven of Intel's, interpolated
#   sudo ./install-ccm.sh revert             remove it again
#
# ONE MATRIX OR SEVEN
#
# With one entry getInterpolated() returns it whatever the light, so a matrix
# fitted under this room's LED is also what a tungsten lamp gets. "intel"
# installs Intel's seven illuminant matrices at the labels this AWB reports for
# them (libcamera/intel-ccms.yaml, generated by extract-ccm.py --yaml), and the
# Ccm algorithm blends between the two nearest on every frame. That is what
# makes the ct label load bearing, and why it is derived rather than typed.
...
if [ "$SPEC" = "intel" ]; then
    SET="$HERE/../libcamera/intel-ccms.yaml"
    [ -f "$SET" ] || { echo "ERROR: $SET is missing" >&2; exit 1; }
    # Re-indent the standalone file to sit under "  - Ccm:". Done here rather
    # than storing it pre-indented so the file stays loadable on its own, which
    # is what lets extract-ccm.py --yaml be diffed against it.
    CCM_BLOCK="$(python3 - "$SET" <<'ENTRIES'
import sys, yaml
d = yaml.safe_load(open(sys.argv[1]))
out = ["  - Ccm:", "      ccms:"]
for e in sorted(d["ccms"], key=lambda e: e["ct"]):
    m = ", ".join("% .5f" % v for v in e["ccm"])
    out += ["        - ct: %d" % e["ct"], "          ccm: [ %s ]" % m]
print("\n".join(out))
ENTRIES
)" || exit 1
    echo "== intel =="
    echo "   $(grep -c '^        - ct:' <<<"$CCM_BLOCK") matrices at $(sed -n 's/^ *- ct: //p' <<<"$CCM_BLOCK" | tr '\n' ' ')K"
else
...
# The interpolator keys a std::map on ct, so duplicate labels lose an entry
# quietly. Cheap to check here, invisible if it ever happens.
cts = [c["ct"] for c in d["algorithms"][algs.index("Ccm")]["Ccm"]["ccms"]]
assert len(set(cts)) == len(cts), f"duplicate ct labels: {cts}"
if "Adjust" in algs:
    assert algs.index("Ccm") < algs.index("Adjust"), \
        "Ccm must come before Adjust, or Saturation is never registered"
print(f"   yaml ok, order ok, {len(cts)} ccm entr{'y' if len(cts)==1 else 'ies'}:",
      " -> ".join(algs))
PY
