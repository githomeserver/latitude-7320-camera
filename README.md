...
| `DENOISE_THR` | `40` | Raw counts of change before a region counts as moving. The noise floor is estimated per frame and subtracted first, so this does not need retuning when the light changes. Each tile's motion is also divided by the lens-shading gain applied there - without that, shading-amplified corner noise crosses this threshold, every corner tile is classified as moving, and the denoise does nothing over most of the frame. |
...
| `THREADS` | physical cores | How many threads split the RGB-IR pre-pass, which is the largest single cost in a frame. The whole software ISP runs on one thread otherwise: measured here it sat at 99.9% of one core while the other seven logical CPUs idled. Bands are horizontal and independent, and the result is **bit identical** to the single-threaded path - `libcamera-rgbir/test-threads.cpp` checks that across 48 combinations of blur, shading, banding and thread count, because the visible symptom of a band reading outside itself would be a seam at every boundary. Measured on this 4-core i5-1140G7, pre-pass and whole frame period: **1 thread 23.5 / 41.0 ms, 2 threads 15.7 / 38.4, 4 threads 11.2 / 35.0, 8 threads 8.9 / 33.6**. The default is the physical core count rather than `hardware_concurrency()`: the pass is limited by memory bandwidth as much as arithmetic and two SMT siblings share one set of load/store units, and 8 threads leaves nothing for the application that is consuming the video. At 8 the frame period reaches the sensor's own 33.67 ms, so there is nothing further to win here. |
...
| `EV` | `0` | Exposure compensation in stops. **Currently inert on this build** - `libcamerasrc` accepts the property, but the camera never advertises `ExposureValue`, so the value is dropped before it reaches the IPA. `cam --list-controls` lists exactly three controls here: `Saturation`, `Contrast`, `Gamma`. Nothing for exposure, gain or white balance. The AGC itself works - the picture is correctly exposed - it simply exposes no knobs. Writing the sensor's own `exposure` control with `v4l2-ctl` is overwritten within a frame, so the only way to force an exposure is to write it in a tight loop while capturing (see the header of `libcamera-rgbir/measure-iradapt.cpp`). Left in place because a later libcamera may register the control. |
